...
//	double* rt = new double [xsize*ysize];
//	double* gt= new double [xsize*ysize];
//	double* bt = new double [xsize*ysize];
	...
	//double* red= new double[xsize * ysize];
	//for (int i =0;i<xsize*ysize;i++){
	//	red[i]=(double)(r[i]);
	//}

	//fftw_complex *out;
	//out = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * xsize*(ysize/2)+1);
	file_in.close();
...
/*	
	...
	for(int i=0; i<2*xsize;i++){
		for(int j=0; j<2*ysize;j++){	
//				r2out[i*(2*xsize)+j]=1/(xsize*ysize)*r2out[i*(2*xsize)+j];
//				g2out[i*(2*xsize)+j]=1/(xsize*ysize)*g2out[i*(2*xsize)+j];
//				b2out[i*(2*xsize)+j]=1/(xsize*ysize)*b2out[i*(2*xsize)+j];
	...
	//ppmb_write(file_out_name, 2*xsize,2*ysize,r2char,g2char,b2char);
